...
export default function useImportExport(
  ...
  const logImport = (
    ...
    if (payload) {
      logger(`[${collectionName}-import] ${message}`, payload)
    } else {
      logger(`[${collectionName}-import] ${message}`)
    }
  }
   ...
   */
  const importFromCSV = async (file: File): Promise<TImportExport.ImportResult> => {
    isImporting.value = true
    const result: TImportExport.ImportResult = {
      processed: 0,
      created: 0,
      updated: 0,
      unchanged: 0,
          ...
          getImportableFields().forEach((fieldConfig: any) => {
            if (
              ...
              (record[fieldConfig.key] === undefined ||
                record[fieldConfig.key] === null ||
                record[fieldConfig.key] === '')
            ) {
              record[fieldConfig.key] = fieldConfig.derive(record)
            }
          })
