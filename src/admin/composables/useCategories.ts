...
export default function useCategories() {
    ...
    const loadCategories = async () => {
        ...
        categories.value = await pb.collection('category').getFullList({
        ...
        categories.value.map(parentCat => {
            if (parentCat.expand?.category_via_Parent) {
                parentCat.expand.category_via_Parent.sort((a, b) => a.tag.localeCompare(b.tag));
            }
        });
    ...
    const saveCategory = async (payload: { tag: string, slug: string, Parent: string | null }) => {
        const data: any = {
            tag: payload.tag,
            slug: payload.slug,
        };
